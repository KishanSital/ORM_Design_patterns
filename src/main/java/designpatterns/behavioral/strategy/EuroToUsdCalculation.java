...
public class EuroToUsdCalculation implements UatmOperation {
    ...
    public BigDecimal executeOperation(BankAccount receiversBankAccount, BigDecimal amountToSend) {
        return receiversBankAccount.getBankBalance().add(amountToSend.multiply(overmaakKoersMap.get("EURO->SRD").divide(overmaakKoersMap.get("USD->SRD"), RoundingMode.UNNECESSARY)));
